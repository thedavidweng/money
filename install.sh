...
# money installer
# Usage: curl -fsSL https://raw.githubusercontent.com/thedavidweng/money/main/install.sh | sh
...
install_binary() {
  ...
  bin_dir="${MONEY_INSTALL_DIR:-$HOME/.local/bin}"
  mkdir -p "$bin_dir"
...
uninstall_brew() {
  step "Uninstalling Homebrew-managed money"
...
case "${1:-}" in
  uninstall)
    if has_brew && brew list --cask "$CASK" >/dev/null 2>&1; then
      ...
      step "Uninstalling legacy Homebrew formula for money"
      brew uninstall --formula "$BINARY"
    else
      uninstall_binary
    fi
    step "Uninstalled. Local config, secrets, and database remain in ~/.money/"
    exit 0
    ;;
  --help|-h)
    cat <<EOF
...
Installs money. Prefers Homebrew Cask if available, otherwise downloads
the binary to ~/.local/bin.

Environment:
  MONEY_INSTALL_DIR  Directory for binary (default: ~/.local/bin)
...
step "Installing money ($platform_label)"
